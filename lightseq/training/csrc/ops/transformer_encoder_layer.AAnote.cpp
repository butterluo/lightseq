...
void TransformerEncoderLayer<T>::attn_layer_fw(const T *input_ptr,
  ...
  const T *gemmQKV_inp_ptr =
      _pre_or_postLayerNorm ? _gemmQKV_inp_ptr : input_ptr;//{buffer=gemmQKV_inp_ptr@_attn_qkvw_ptr}
  ...
  launch_bias_add_transform_20314<T>(q_tf_ptr, buffer, _attn_qkvb_ptr,//q_tf_ptr指向[3, batchSz, heads, seqlen, headDim]的开端,往后移动_batch_dim就是k_tf_ptr,再移动_batch_dim就是v_tf_ptr
                                     ...
                                     _hidden_size / _heads, _stream);//{q_tf_ptr=buffer@_attn_qkvb_ptr}{buffer,_attn_qkvb_ptr,,k_tf_ptr,,input_mask_ptr,,v_tf_ptr,,_attn_ow_ptr,,input_ptr,_attn_ob_ptr,,_attn_nw_ptr,_attn_nb_ptr}

  // attention scores, q*k
  _attn_scores.Forward(_batch_heads, _soft_out_ptr, k_tf_ptr, q_tf_ptr,
                       _cublasHandle);//{_soft_out_ptr=k_tf_ptr@q_tf_ptr}}{k_tf_ptr,q_tf_ptr,,input_mask_ptr,,v_tf_ptr,,_attn_ow_ptr,,input_ptr,_attn_ob_ptr,,_attn_nw_ptr,_attn_nb_ptr}

  // Softmax + Mask
  _softmax.Forward(_soft_out_ptr, input_mask_ptr, _batch_size, _seq_len,
                   _seq_len, _stream);//{_soft_out_ptr=_soft_out_ptr@input_mask_ptr}{input_mask_ptr,,v_tf_ptr,,_attn_ow_ptr,,input_ptr,_attn_ob_ptr,,_attn_nw_ptr,_attn_nb_ptr}

  // attn prob dropout.
  _attn_prob_dropout.dropout(_ctx_bufB_ptr, _soft_out_ptr,
                             _batch_heads * _seq_len * _seq_len, _stream);//{_ctx_bufB_ptr=_soft_out_ptr@}{_soft_out_ptr,,v_tf_ptr,,_attn_ow_ptr,,input_ptr,_attn_ob_ptr,,_attn_nw_ptr,_attn_nb_ptr}

  // attention context, score * v
  _attn_context.Forward(_batch_heads, buffer, v_tf_ptr, _ctx_bufB_ptr,
                        _cublasHandle);//{buffer=v_tf_ptr@_ctx_bufB_ptr}{v_tf_ptr,_ctx_bufB_ptr,,_attn_ow_ptr,,input_ptr,_attn_ob_ptr,,_attn_nw_ptr,_attn_nb_ptr}

  // [b, nh, s, ad] -> [b, s, nh, ad]
  launch_transform4d_0213<T>(_attn_o_inp_ptr, buffer, _batch_size, _seq_len,
                             _hidden_size, _heads, 1, _stream);//{_attn_o_inp_ptr=buffer@}{buffer,,_attn_ow_ptr,,input_ptr,_attn_ob_ptr,,_attn_nw_ptr,_attn_nb_ptr}

  _attn_out_linear.Forward(_batch_tokens, _attn_o_inp_ptr, _attn_ow_ptr,
                           output_ptr, _cublasHandle);//{output_ptr=_attn_o_inp_ptr@_attn_ow_ptr}{_attn_o_inp_ptr,_attn_ow_ptr,,input_ptr,_attn_ob_ptr,,_attn_nw_ptr,_attn_nb_ptr}

  _attn_dropout.bias_dropout_residual(output_ptr, output_ptr, input_ptr,
                                      _attn_ob_ptr, _batch_tokens, _hidden_size,
                                      _stream); //{output_ptr=output_ptr@input_ptr@_attn_ob_ptr}{output_ptr,input_ptr,_attn_ob_ptr,,_attn_nw_ptr,_attn_nb_ptr}
  if (!_pre_or_postLayerNorm) {
    // in-place ln since ln-input will not be used in post-ln mode  {output_ptr=output_ptr@_attn_nw_ptr@_attn_nb_ptr}{output_ptr,_attn_nw_ptr,_attn_nb_ptr}
...
void TransformerEncoderLayer<T>::attn_layer_bw(const T *input_ptr,
  ...
  launch_transform_0213<T>(grad_input_ptr, grad_input_buf_ptr, _batch_size,
                           _seq_len, _hidden_size, _heads, _stream);//{grad_input_ptr=grad_input_buf_ptr@}

  // bw of score * v
  _attn_context.Backward(_batch_heads, grad_input_ptr, v_tf_ptr, _ctx_bufB_ptr,
                         _cublasHandle, grad_qkv_5d_ptr + 2 * _batch_dim,
                         grad_softmax_ptr);//{grad_qkv_5d_ptr+2*_batch_dim=grad_input_ptr@_ctx_bufB_ptr, grad_softmax_ptr=grad_input_ptr@v_tf_ptr}

  _attn_prob_dropout.d_dropout(grad_softmax_ptr,
                               _batch_heads * _seq_len * _seq_len, _stream);//{grad_softmax_ptr=grad_softmax_ptr@}

  _softmax.Backward(grad_softmax_ptr, _soft_out_ptr, _batch_size, _seq_len,
                    _seq_len, _stream);//{grad_softmax_ptr=_soft_out_ptr@}

  // bw of q * k
  _attn_scores.Backward(_batch_heads, grad_softmax_ptr, k_tf_ptr, q_tf_ptr,
                        _cublasHandle, grad_qkv_5d_ptr + _batch_dim,
                        grad_qkv_5d_ptr);//{grad_qkv_5d_ptr+_batch_dim=grad_softmax_ptr@q_tf_ptr, grad_qkv_5d_ptr=grad_softmax_ptr@k_tf_ptr}

  // [3, b, nh, s, ad] -> [b, s, 3, h]
  launch_transform4d_0213<T>(grad_qkv_4d_ptr, grad_qkv_5d_ptr, _batch_size,
                             _seq_len, _hidden_size, _heads, 3, _stream);//{grad_qkv_4d_ptr=grad_qkv_5d_ptr@}
  ...
  _qkv_linear.Backward(_batch_tokens, grad_qkv_4d_ptr, gemmQKV_inp_ptr,
                       ...
                       _cublasHandle, _stream, grad_input_buf_ptr);//{_grad_attn_qkvw_ptr=grad_qkv_4d_ptr@gemmQKV_inp_ptr, grad_input_buf_ptr=_attn_qkvw_ptr@grad_qkv_4d_ptr, bias_grad=grad_qkv_4d_ptr@}
  ...
  if (_pre_or_postLayerNorm) {
    ...
    launch_fused_add2<T>(grad_input_ptr, grad_input_buf_ptr, grad_residual_ptr,
                         _batch_size, _seq_len, _hidden_size, _stream);//{grad_input_ptr=grad_input_buf_ptr@grad_residual_ptr}
  }
}
